def eigenfrequency_anisotropic(assumeddeflection, material, settings, stiffness, data=False, illustrations=False):
    ...
    for i in range(situations):
        ...
        o =     D[0, 0] *  ddXkddXm[:, :, BC[i, 0] ]   *     YlYn[:, :, BC[i, 1] ]    \
          ...
          + 2 * D[0, 2] * ( ddXkdXm[:, :, BC[i, 0] ]   *    YldYn[:, :, BC[i, 1] ]    \
          +                 ddXkdXm[:, :, BC[i, 0] ].T *    YldYn[:, :, BC[i, 1] ].T) \
          + 2 * D[1, 2] * (   XkdXm[:, :, BC[i, 0] ].T *  ddYldYn[:, :, BC[i, 1] ].T  \
          +                   XkdXm[:, :, BC[i, 0] ]   *  ddYldYn[:, :, BC[i, 1] ])
