def MaximumStressFailureTheoryEnvelope(laminate, material, settings, tqad, data=False, illustrations=False):
    ...
    from   numpy             import argmin, array, concatenate, deg2rad, einsum, errstate, flip, linspace, ones, pi, repeat, sin, sort, \
                                    sqrt, tan, tile, where, zeros, copy, inf, isinf
    ...
    def SingleFailureLoad(d, tqad, X_c, X_t, Y_c, Y_t, S):
        ...
        N_x = zeros(2)
        # N_x_neg
        N_x[0] = min( (tqad[0, d, :] < 0) * (X_t / tqad[0, d, :] ) - (tqad[0, d, :] > 0) * (X_c / tqad[0, d, :] ) + (tqad[0, d, :] == 0) * 0, key = abs)
        # N_x_pos
        N_x[1] = min( (tqad[0, d, :] > 0) * (X_t / tqad[0, d, :] ) - (tqad[0, d, :] < 0) * (X_c / tqad[0, d, :] ) + (tqad[0, d, :] == 0) * 0, key = abs)

        #
        N_y = zeros(2)
        # N_y_neg
        N_y[0] = min( (tqad[1, d, :] < 0) * (Y_t / tqad[1, d, :] ) - (tqad[1, d, :] > 0) * (Y_c / tqad[1, d, :] ) + (tqad[1, d, :] == 0) * 0, key = abs)
        # N_y_pos
        N_y[1] = min( (tqad[1, d, :] > 0) * (Y_t / tqad[1, d, :] ) - (tqad[1, d, :] < 0) * (Y_c / tqad[1, d, :] ) + (tqad[1, d, :] == 0) * 0, key = abs)

        #
        N_xy = zeros(2)
        # N_xy_neg
        N_xy[0] = min( (tqad[2, d, :] < 0) * (S   / tqad[2, d, :] ) - (tqad[2, d, :] > 0) * (S   / tqad[2, d, :] ) + (tqad[2, d, :] == 0) * 0, key = abs)
        # N_xy_pos
        N_xy[1] = min( (tqad[2, d, :] > 0) * (S   / tqad[2, d, :] ) - (tqad[2, d, :] < 0) * (S   / tqad[2, d, :] ) + (tqad[2, d, :] == 0) * 0, key = abs)
    ...
    ##
    def CylindricalCoordinateSolver(data_points, d1, d2, s1, N, theta, ratio_force, tqad, F_t, F_c):

        #
        coefficient_force_min = (einsum('i,j->ij', tqad[s1, d1, :], ones(data_points) ) - einsum('i,j->ij', tqad[s1, d2, :], ratio_force) )
        coefficient_force_max = (einsum('i,j->ij', tqad[s1, d1, :], ones(data_points) ) + einsum('i,j->ij', tqad[s1, d2, :], ratio_force) )
    ...
    ##
    def FailureEnvelope(data_points, N_x, N_y, N_xy, d1, d2, tqad, X_c, X_t, Y_c, Y_t, S):
        ...
        #
        MN_x  = CylindricalCoordinateSolver(data_points, d1, d2, 0, N_x, theta, ratio_force, tqad, X_t, X_c)

        #
        MN_y  = CylindricalCoordinateSolver(data_points, d1, d2, 1, N_y, theta, ratio_force, tqad, Y_t, Y_c)

        #
        MN_xy = CylindricalCoordinateSolver(data_points, d1, d2, 2, N_xy, theta, ratio_force, tqad, S, S)

        #
        mn       = zeros( [data_points + 1, 3] )
        mn[:, 0] = norm(MN_x,  axis=1)
        mn[:, 1] = norm(MN_y,  axis=1)
        mn[:, 2] = norm(MN_xy, axis=1)

        index_min = argmin(mn, axis=1)

        MN       = zeros( [data_points + 1, 2] )
        MN[:, 0] = (index_min == 0) * MN_x[:, 0] + (index_min == 1) * MN_y[:, 0] + (index_min == 2) * MN_xy[:, 0]
        MN[:, 1] = (index_min == 0) * MN_x[:, 1] + (index_min == 1) * MN_y[:, 1] + (index_min == 2) * MN_xy[:, 1]
    ...
    for i in range(0, number_of_forces):
        ...
        locals()[f'{forces[i]}_x'], locals()[f'{forces[i]}_y'], locals()[f'{forces[i]}_xy'], locals()[forces[i] ] \
            = SingleFailureLoad(i, tqad, X_c, X_t, Y_c, Y_t, S)
        #
        MaximumStress[forces[i] ] = locals()[forces[i] ]
        ...
        for j in range(i + 1, number_of_forces):
            ...
            locals()[f'{forces[i]}{forces[j]}_x'], locals()[f'{forces[i]}{forces[j]}_y'], locals()[f'{forces[i]}{forces[j]}_xy'], locals()[f'{forces[i]}{forces[j]}'] \
                = FailureEnvelope(data_points, locals()[f'{forces[j]}_x'], locals()[f'{forces[j]}_y'], locals()[f'{forces[j]}_xy'], i, j, \
                                  tqad, X_c, X_t, Y_c, Y_t, S)
            #
            MaximumStress[forces[i] + forces[j] ] = locals()[f'{forces[i]}{forces[j]}']

    #
    if data:
                #
        if exists(f'{simulation}/Data/MaterialFailureEnvelopes/MaximumStressFailureTheory.txt'):
            remove(f'{simulation}/Data/MaterialFailureEnvelopes/MaximumStressFailureTheory.txt')

        #
        textfile = open(f'{simulation}/Data/MaterialFailureEnvelopes/MaximumStressFailureTheory.txt', 'w', encoding='utf8')

        #
        textfile.write(fill(f'Singular failure loads according to the Maximum Stress Failure Theory') )

        #
        textfile.write(f'\n\n')

        #
        textfile.write(fill(f'Source: MaximumStressFailureTheoryEnvelope.py [v1.0] via CompositeAnalysis.py [v1.0] ({datetime.now().strftime("%H:%M:%S %d-%m-%Y")}).') )
        ...
        textfile.write(tabulate( array( [ [f'N\u2093  [N/m]', locals()[f'{forces[0]}_x'][0],  locals()[f'{forces[0]}_x'][1], \
                    ...
                    headers=(f'Force', f'\u03C3\u2081', f'', f'\u03C3\u2082', f'', f'\u03C4\u2081\u2082', f'', f'Maximum Stress', f'Failure Theory'),
    ...
    if illustrations:
        ...
        # Labels
        Labels = [f'N\u2093 [N]', f'N\u1D67 [N]', f'N\u2093\u1D67 [N]', f'M\u2093 [N]', f'M\u1D67 [N]', f'M\u2093\u1D67 [N]']

        #
        if not isdir(f'{simulation}/Illustrations/MaterialFailureEnvelopes/MaximumStressFailureTheory'):
            mkdir(f'{simulation}/Illustrations/MaterialFailureEnvelopes/MaximumStressFailureTheory')
        ...
        for i in range(0, 5):
            ...
            for j in range(i + 1, 6):
                #
                if exists(f'{simulation}/Illustrations/MaterialFailureEnvelopes/MaximumStressFailureTheory/{forces[i]}{forces[j]}.{fileformat}'):
                    remove(f'{simulation}/Illustrations/MaterialFailureEnvelopes/MaximumStressFailureTheory/{forces[i]}{forces[j]}.{fileformat}')
                ...
                # Create plots
                plt.plot(locals()[f'{forces[i]}{forces[j]}'][:, 1],    locals()[f'{forces[i]}{forces[j]}'][:, 0],    color=Colors[3],  linewidth=1, linestyle=LineStyles[0], label=f'Maximum Stress Failure Theory')
                plt.plot(locals()[f'{forces[i]}{forces[j]}_x'][:, 1],  locals()[f'{forces[i]}{forces[j]}_x'][:, 0],  color=Colors[21], linewidth=1, linestyle=LineStyles[2], label=f'\u03C3\u2081')
                plt.plot(locals()[f'{forces[i]}{forces[j]}_y'][:, 1],  locals()[f'{forces[i]}{forces[j]}_y'][:, 0],  color=Colors[22], linewidth=1, linestyle=LineStyles[2], label=f'\u03C3\u2082')
                plt.plot(locals()[f'{forces[i]}{forces[j]}_xy'][:, 1], locals()[f'{forces[i]}{forces[j]}_xy'][:, 0], color=Colors[23], linewidth=1, linestyle=LineStyles[2], label=f'\u03C4\u2081\u2082')
                #
                plt.plot(0, locals()[forces[i] ][0], color=Colors[3], marker='.')
                plt.plot(0, locals()[forces[i] ][1], color=Colors[3], marker='.')
                plt.plot(locals()[forces[j] ][0], 0, color=Colors[3], marker='.')
                plt.plot(locals()[forces[j] ][1], 0, color=Colors[3], marker='.')
                #
                ...
                plt.text(0, -0.15, f'Source: MaximumStressFailureTheory.py [v1.0] via CompositeAnalysis.py [v1.0] ({datetime.now().strftime("%H:%M:%S %d-%m-%Y")}).', fontsize = 6, horizontalalignment='left', verticalalignment='center', transform=plt.gca().transAxes)
                # Save figure
                plt.savefig(f'{simulation}/Illustrations/MaterialFailureEnvelopes/MaximumStressFailureTheory/{forces[i]}{forces[j]}.{fileformat}', bbox_inches='tight')
                # Close figure
                plt.close(0)
