def PressureLoad_MidPlaneSymmetric(assumeddeflection, force, geometry, mesh, settings, stiffness, data=False, illustrations=False):
    ...
    from numpy           import reshape, real, sum, cos, cosh, sin, sinh, imag, arange, array, copy, einsum, linspace, sin, tile, ones, zeros
    ...
    # Save figure
    plt.savefig(f'{simulation}/Illustrations/PressureLoad/ErrorConvergenceRitz_MPS.{fileformat}', bbox_inches="tight")
    # Close figure
    plt.close(0)
    ...
    plt.text(0, -0.15, f'Source: PressureLoad_SpeciallyOrthotropic.py [v1.0] via CompositeAnalysis.py [v1.0] ({datetime.now().strftime("%H:%M:%S %d-%m-%Y")}).', fontsize = 6, horizontalalignment='left', verticalalignment='center', transform=plt.gca().transAxes)
    # Save figure
    plt.savefig(f'{simulation}/Illustrations/PressureLoad/DisplacementConvergenceRitz_MPS.{fileformat}', bbox_inches="tight")
    # Close figure
    plt.close(0)
    ...
    if data:
        #
        if exists(f'{simulation}/Data/PressureLoad/MaximumDeformation_MPS.txt'):
            remove(f'{simulation}/Data/PressureLoad/MaximumDeformation_MPS.txt')

        #
        textfile = open(f'{simulation}/Data/PressureLoad/MaximumDeformation_MPS.txt', 'w', encoding='utf8')
