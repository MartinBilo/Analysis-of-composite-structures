def PressureLoad_SpeciallyOrthotropic(assumeddeflection, force, geometry, mesh, settings, stiffness, data=False, illustrations=False):
    ...
    from   numpy             import arange, array, copy, cos, cosh, einsum, empty, imag, linspace, ones, real, reshape, sin, sinh, sum, \
                                    tile, zeros
    ...
    #
    error_Lévy_simplified    = zeros( [mn_max, Situations_Lévy] )
    error_Lévy_simplified[0] = inf
    ...
    #
    delta_Lévy_simplified = zeros( [mn_max, Situations_Lévy] )
    ...
    #
    mn_Lévy_simplified = zeros(Situations_Lévy, dtype = int)
    ...
    #
    x_Lévy_simplified = zeros(Situations_Lévy)
    y_Lévy_simplified = zeros(Situations_Lévy)
    ...
    #
    delta_Ritz_analytical = zeros(4)

    # (simply supported beam)
    delta_Ritz_analytical[0] = 4 * p_z / (pi**6 * (D[0,0] / (4 * a**4) + (D[0,1] + 2 * D[2,2]) / (2 * a**2 * b**2) + D[1,1] / (4 * b**4) ) )

    # clamped simply supported
    # delta_Ritz_analytical[1] =

    # Equation 5.58 (clamped beam)
    delta_Ritz_analytical[2] = 0.00348 * p_z * a**4 / (D[0,0] + 0.6047 * (D[0,1] + 2 * D[2,2]) * a**2 / b**2 + D[1,1] * a**4 / b**4)

    # Equation 5.48 (clamped polynomial)
    delta_Ritz_analytical[3] = 0.00342 * p_z * a**4 / (D[0,0] + 0.571 * (D[0,1] + 2 * D[2,2]) * a**2 / b**2 + D[1,1] * a**4 / b**4)


    ## Simplified Lévy solution
    ...
    # Equation 5.33 from [1] and expression X.XX from []
    q_mn = 2 * p_z * (1 - cos(N * pi) ) / (N * pi)

    # Equation 5.33 from [1] and expression X.XX from []
    D_mn = (BC_Lévy == 0) * D[1, 1] + (BC_Lévy == 1) * D[0, 0]

    # Equation 5.34 from [1]
    w_p = einsum('i,j->ij', q_mn / (N**4 * pi**4), width**4 / D_mn)
    ...
    for o in range(Situations_Lévy):
        ...
        if o == 0 or o == 1:
            A_n, B_n, C_n, D_n \
                = SimplifiedLévyCoefficientsSimplySupportedSimplySupported(beta, Lambda_1[o], Lambda_2[o], length[o], width[o], w_p[:,o], N)
        elif o == 2 or o == 3:
            A_n, B_n, C_n, D_n \
                = SimplifiedLévyCoefficientsClampedSimplySupported(beta, Lambda_1[o], Lambda_2[o], length[o], width[o], w_p[:,o], N)
        elif o == 4 or o == 5:
            A_n, B_n, C_n, D_n \
                = SimplifiedLévyCoefficientsClampedClamped(beta, Lambda_1[o], Lambda_2[o], length[o], width[o], w_p[:,o], N)
        elif o == 6 or o == 7:
            A_n, B_n, C_n, D_n \
                = SimplifiedLévyCoefficientsFreeFree(N)
        elif o == 8 or o == 9:
            A_n, B_n, C_n, D_n \
                = SimplifiedLévyCoefficientsFreeSimplySupported(beta, Lambda_1[o], Lambda_2[o], length[o], width[o], w_p[:,o], N)
        elif o == 10 or o == 11:
            A_n, B_n, C_n, D_n \
                = SimplifiedLévyCoefficientsClampedFree(beta, Lambda_1[o], Lambda_2[o], length[o], width[o], w_p[:,o], N)

        nodes_X = (BC_Lévy[o] == 0) * nodes_x + (BC_Lévy[o] == 1) * nodes_y
        nodes_Y = (BC_Lévy[o] == 0) * nodes_y + (BC_Lévy[o] == 1) * nodes_x

        #
        while error_Lévy_simplified[counter, o] >= margin and mn <= mn_max:
            ...
            w = einsum('ijk,ijk->ij', phi_n,      sin(einsum('k,ij->ijk',   N[0:mn] * pi / width[o], nodes_Y) ) ) \
              + einsum('k,ijk->ij', w_p[0:mn, o], sin(einsum('k,ij->ijk',   N[0:mn] * pi / width[o], nodes_Y) ) )

            # Maximum deflection
            delta_Lévy_simplified[counter, o] = max(w.flatten(), key=abs)

            # Normalized, maximum norm
            if counter == 0:
                error_Lévy_simplified[counter, o] = 100
            else:
                error_Lévy_simplified[counter, o] = real(sqrt( (w - W).flatten() @ (w - W).flatten() / (W.flatten() @ W.flatten() ) ) ) * 100
        ...
        #
        mn_Lévy_simplified[o] = mn - 2

        #
        x_Lévy_simplified[o] = median(nodes_x[w == delta_Lévy_simplified[counter, o] ] )
        y_Lévy_simplified[o] = median(nodes_y[w == delta_Lévy_simplified[counter, o] ] )
    ...
    if illustrations:
        ...
        #
        if exists(f'{simulation}/Illustrations/PressureLoad/ErrorConvergenceNavier_SpeciallyOrthotropic.{fileformat}'):
            remove(f'{simulation}/Illustrations/PressureLoad/ErrorConvergenceNavier_SpeciallyOrthotropic.{fileformat}')

        #
        if exists(f'{simulation}/Illustrations/PressureLoad/ErrorConvergenceRitz_SpeciallyOrthotropic.{fileformat}'):
            remove(f'{simulation}/Illustrations/PressureLoad/ErrorConvergenceRitz_SpeciallyOrthotropic.{fileformat}')

        #
        if exists(f'{simulation}/Illustrations/PressureLoad/ErrorConvergenceLévySimplified_SpeciallyOrthotropic.{fileformat}'):
            remove(f'{simulation}/Illustrations/PressureLoad/ErrorConvergenceLévySimplified_SpeciallyOrthotropic.{fileformat}')

        #
        if exists(f'{simulation}/Illustrations/PressureLoad/ErrorConvergenceLévy_SpeciallyOrthotropic.{fileformat}'):
            remove(f'{simulation}/Illustrations/PressureLoad/ErrorConvergenceLévy_SpeciallyOrthotropic.{fileformat}')
        ...
        plt.xlabel(f'M = N [-]')
        plt.xlim(1, max(mn_Navier, max(mn_Ritz), max(mn_Lévy_simplified), max(mn_Lévy) ) )
        plt.xticks(arange(1, (max(mn_Navier, max(mn_Ritz), max(mn_Lévy_simplified), max(mn_Lévy) ) + 2), 2) )
        # Create legend
        ...
        plt.text(0, -0.15, f'Source: PressureLoad_SpeciallyOrthotropic.py [v1.0] via CompositeAnalysis.py [v1.0] ({datetime.now().strftime("%H:%M:%S %d-%m-%Y")}).', fontsize = 6, horizontalalignment='left', verticalalignment='center', transform=plt.gca().transAxes)
        ...
        # Create plots
        for o in range(Situations_Lévy):
            plt.plot(linspace(1, mn_Lévy_simplified[o], (mn_Lévy_simplified[o] + 1) // 2), error_Lévy_simplified[0:(mn_Lévy_simplified[o] + 1) // 2, o],
        ...
        #
        yb = min(y_b0, y_b1, y_b2, y_b3)
        yt = max(y_t0, y_t1, y_t2, y_t3)
        ...
        plt.ylim(yb, yt)
        # Save figure
        plt.savefig(f'{simulation}/Illustrations/PressureLoad/ErrorConvergenceNavier_SpeciallyOrthotropic.{fileformat}', bbox_inches="tight")
        # Close figure
        plt.close(0)
        ...
        plt.ylim(yb, yt)
        # Save figure
        plt.savefig(f'{simulation}/Illustrations/PressureLoad/ErrorConvergenceRitz_SpeciallyOrthotropic.{fileformat}', bbox_inches="tight")
        # Close figure
        plt.close(1)
        ...
        plt.ylim(yb, yt)
        # Save figure
        plt.savefig(f'{simulation}/Illustrations/PressureLoad/ErrorConvergenceLévySimplified_SpeciallyOrthotropic.{fileformat}', bbox_inches="tight")
        # Close figure
        plt.close(2)
        ...
        plt.ylim(yb, yt)
        # Save figure
        plt.savefig(f'{simulation}/Illustrations/PressureLoad/ErrorConvergenceLévy_SpeciallyOrthotropic.{fileformat}', bbox_inches="tight")
        # Close figure
        plt.close(3)
    ...
    if data:
        #
        if exists(f'{simulation}/Data/PressureLoad/MaximumDeformation_SpeciallyOrthotropic.txt'):
            remove(f'{simulation}/Data/PressureLoad/MaximumDeformation_SpeciallyOrthotropic.txt')

        #
        textfile = open(f'{simulation}/Data/PressureLoad/MaximumDeformation_SpeciallyOrthotropic.txt', 'w', encoding='utf8')
        ...
        #
        textfile.write(fill(f'Source: PressureLoad_SpeciallyOrthotopic.py [v1.0] via CompositeAnalysis.py [v1.0] ({datetime.now().strftime("%H:%M:%S %d-%m-%Y")}).') )
        ...
        #
        textfile.write(f'Simplified Lévy solution \n')

        #
        textfile.write(tabulate( array([BoundaryConditions_Lévy, delta_Lévy_simplified[(mn_Lévy_simplified - 1) // 2, arange(delta_Lévy_simplified.shape[1]) ], x_Lévy_simplified, y_Lévy_simplified, mn_Lévy_simplified, error_Lévy_simplified[(mn_Lévy_simplified - 1) // 2, arange(error_Lévy_simplified.shape[1]) ] ] ).T.tolist(),
                    headers=(f'Boundary conditions', f'\u03B4\u2098\u2090\u2093 [m]', f'x [m]', f'y [m]', f'm = n [-]', f'\u03B5 [%]'),
                    stralign=('left'),
                    numalign=('decimal'),
                    floatfmt=('.5e', '.5e', '.2e', '.2e', '.0f', '.5e') ) )
